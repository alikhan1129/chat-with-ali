...
export default function ChatMessageContent({
  ...
  // Helper to render a block of text with markdown and code blocks
    ...
    if (message.parts && message.parts.length > 0) {
      return message.parts.map((part, index) => {
        if (part.type === 'text' && part.text) {
          return renderTextBlocks(part.text, `part-${index}`);
        }
        return null;
      });
    }

    // Fallback: If no parts, render the raw content
    if (message.content) {
      return renderTextBlocks(message.content, 'raw-content');
    }
